...
export const userAPI = {
  getAll: (params?: { limit?: number; offset?: number; email?: string; name?: string; contactPhone?: string }) =>
    api.get('/api/admin/users', { params }),
  create: (data: { email: string; password: string; name: string; contactPhone?: string; role?: string }) =>
    api.post('/api/admin/users', data),
};

export const bookAPI = {
  search: (params?: { library?: number; author?: string; title?: string; availableOnly?: boolean }) =>
    api.get('/api/common/books', { params }),
  getById: (id: number) => api.get(`/api/common/books/${id}`),
  create: (data: FormData) => api.post('/api/admin/books', data, {
